...
3) Для размера 2000: при 8 потоках ускорение 5.55 (69% от идеала)
	При увеличении размера эффективность растет
	На малых размерах (200) накладные расходы на создание потоков заметнее
4.2
...
а) Конкуренции за доступ к памяти
б) Накладных расходов на синхронизацию
в) Ограничений пропускной способности памяти
4.3
Закон Амдала
Максимальное теоретическое ускорение ограничено последовательной частью кода:
В данной реализации последовательная часть - только чтение/запись файлов
Основные вычисления полностью распараллелены
Поэтому ускорение близко к идеальному для 2-4 потоков
